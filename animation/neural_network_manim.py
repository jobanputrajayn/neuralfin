...
class NeuralNetworkMobject(VGroup):
    ...
    def __init__(self, neural_network, *args, **kwargs):
        ...
        # Fit to screen after construction
        margin = 1.0  # units to leave as margin
        self.scale_to_fit_width(config.frame_width - margin)
        if self.height > (config.frame_height - margin):
            self.scale_to_fit_height(config.frame_height - margin)
        self.move_to(ORIGIN)
